...
**Network.** `Net.post` first tries a direct `fetch`, with an automatic service-worker fallback only for a network failure before receiving a response. A broken response stream must not be mistaken for CORS rejection. A two-minute deadline covers the entire request, including its body; timeout is retryable, while user cancellation is not. Completion releases abort listeners and stream reader locks. The service worker checks the origin's host permission, relays chunks, and aborts when the port closes. Port messages count as worker activity, but a provider that stays silent can still outlast the worker's idle lifetime; streaming alone is not a lifetime guarantee. Custom domains are granted from Settings through `optional_host_permissions`.

**Caption bridge safeguards.** A captured body must contain actual `json3` text events, not just begin with a JSON brace. Refetches have a 3.5-second deadline. Navigation stops further fallback attempts, and the temporary CC setting is restored only while the original video and player are still current. These checks are covered by simulated bridge tests; they do not prove current YouTube endpoint compatibility.
